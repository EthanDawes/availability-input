<script lang="ts">
    ...
    export let availability: InternalAvailability = blankAvailability(
        UTCDates.map(canonicalDateStr),
    )
    ...
    function applyDragPreview() {
        for (const date in availability) {
            ...
            for (const block of range(0, DAY / TIME_STEP)) {
                const includeBlock =
                    selectRectIncludesBlock(
                        [new Date(date).getTime(), block],
                        dragStart,
                        dragNow,
                    ) && datetimeInRange(ranges, offsetDate(date, block * TIME_STEP))
                if (includeBlock) availability[date][block] = dragState ? ["me"] : []
            }
        }
    }
    ...
    function convertTouchEvent(ev: TouchEvent): [DateStr | undefined, number | undefined] {
        if (ev.touches.length > 1) handlePointerUp()
            ...
            if (!target || !target.classList.contains("availability-cell"))
                return [undefined, undefined]
            const idx = Number.parseInt(target.dataset.idx!)
            const date = Number.parseInt(target.dataset.date!)
            if (!dragStart && !availablePeople && !isDisabled) dragStart = dragNow = [date, idx]
    ...
    const handlePointerEnter = (date?: DateStr, timeIndex?: number) => {
        ...
        if (availablePeople) availablePeople.set(availability[date][timeIndex] ?? [])
    }
            ...
            {#each dates as localDate}
                ...
                {#if datetimeInRange(ranges, offsetDate(date, blockIndex * TIME_STEP))}
                    <div
                        ...
                        style:--lightness={allParticipants.length && !useMulticolor
                            ? (1 -
                                  (colAvailability[blockIndex]?.length ?? allParticipants.length) /
                        ...
                        class:available={selectRectIncludesBlock(
                            [date.getTime(), blockIndex],
                            dragStart,
                            dragNow,
                        )
                            ? dragState
                            : colAvailability[blockIndex]?.length}
                    ...
                    >
                        {#if allParticipants.length && useMulticolor}
                            {#each colAvailability[blockIndex] ?? [] as participant}
                                {@const hue =
                                    ((allParticipants.indexOf(participant) + 1) /
                                        allParticipants.length) *
                                    365}
                                <div
                                    class="flex-1"
                                    style:background-color={`hsl(${hue}, 65%, 79%)`}
                                ></div>
                            {/each}
                        {/if}
                    </div>
